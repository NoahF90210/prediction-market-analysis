...
function EvidenceTable() {
  return <div className="evidence">
    <div className="evidence-head"><div><div className="eyebrow">Evidence sample</div><h3>Real rows behind the result</h3></div><p>This is a small linked sample from the full 75,036-row dataset.</p></div>
...
function App() {
  ...
  return <>
    ...
    <main className="page">
      ...
      <section><SectionHead title="Method and analysis scope" description="The project makes the timing rule, outcome definition, coverage, and market-level evidence visible in one place." /><details><summary>Open methodology and analysis scope</summary><div className="details-body"><div><h3>Method</h3><ul><li>{data.method.snapshot}</li><li>{data.method.buckets}</li><li>{data.method.gap}</li><li>{data.method.outcome}</li></ul></div><div><h3>Coverage</h3><p>{s.included_count.toLocaleString()} of {s.submitted_count.toLocaleString()} YES/NO candidates had usable price history.</p><p>{data.exclusions.map((item) => `${item.count.toLocaleString()} ${item.reason}`).join("; ")}.</p></div><div><h3>Analysis scope</h3><ul>{data.limitations.map((item) => <li key={item}>{item}</li>)}</ul></div></div></details></section>
      <footer><span>Frozen dataset {data.build_id.slice(0, 12)}</span><span>Source SHA-256 {data.source.sha256.slice(0, 12)}</span><span>Local verified build</span><a href="https://github.com/NoahF90210/prediction-market-analysis" target="_blank" rel="noreferrer">View source on GitHub ↗</a></footer>
    </main>
  </>;
}
